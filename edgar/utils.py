"""Shared utility helpers for date parsing and formatting."""
...
class EdgarUtilities():
    ...
    def clean_directories(self, directories: list, cik: str = None) -> list[dict]:
    ...
    def clean_filing_directory(self, directory: list, cik: str = None) -> list[dict]:
        """Used to clean the directories and add additional fields.
    ...
    def parse_dates(self, date_or_datetime: Union[date, datetime, str]) -> str:
        ...
        if isinstance(date_or_datetime, str):
            try:
                ...
                raise ValueError(
                    f"Date string '{date_or_datetime}' is not in ISO format (YYYY-MM-DD)."
                ) from exc
        else:
            raise ValueError(
